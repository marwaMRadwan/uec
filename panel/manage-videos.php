...
if($_GET[pro]=="") header('Location:manage-tour_hous.php.php');
...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    foreach ($_POST as $name => $value) {
        ...
        $i=substr($name,4); 
        if($t=="dele"){
            $q="DELETE FROM `videos` WHERE `id`='$i'";
...
echo '<h2>Manage '.$r[name].'Videos</h2>';
}?>
            ...
            <table id="datatable" class="table table-striped table-bordered">
				...
				<tbody>
              		<?php
                       $q="SELECT * FROM `videos` WHERE `proid`='$_GET[pro]'";
                       $qrun=mysqli_query($connection,$q);
