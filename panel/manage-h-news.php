...
if($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $i=substr($name,4); 
        if($t=="dele"){
            $q="DELETE FROM `h_articles` WHERE `id`='$i'";
...
<div class="row">
	<div class="col-md-12 col-sm-12 col-xs-12">
		<div class="x_panel">
    		<div class="x_title">
<?php 
				$qa="SELECT `id`, `enname` FROM `housing` WHERE `id`='$_GET[pro]'";
$qarun=mysqli_query($connection,$qa);
if($r=mysqli_fetch_array($qarun)){
echo '<h2>Manage '.$r[enname].'Videos</h2>';
}?>
            ...
            <table id="datatable" class="table table-striped table-bordered">
				...
				<tbody>
              		<?php
                       $q="SELECT * FROM `h_articles` WHERE `proid`='$_GET[pro]'";
                       $qrun=mysqli_query($connection,$q);
